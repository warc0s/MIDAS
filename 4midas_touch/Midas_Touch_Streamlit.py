...
from Midas_Touch_V2_CLI import (
...
def main():
    ...
    with st.sidebar:
        ...
        user_input = st.text_area("Describir tarea de ML", 
                               placeholder="Ej: 'predecir riesgo de diabetes'",
                               height=100)
    ...
    if st.session_state.workflow_completed:
        ...
        tab1, tab2, tab3 = st.tabs(["📔 Notebook", "📥 Descargas", "📝 Logs"])
        
        with tab1:
            # Mostrar contenido del notebook si existe
            ...
            if notebook_path and os.path.exists(notebook_path):
                ...
                st.markdown("### Vista previa del Notebook")
                st.caption("(Las celdas se muestran con formato simplificado)")
                
                # Mostrar las primeras 10 celdas con formato mejorado
                cells = notebook.get('cells', [])[:10]
                for i, cell in enumerate(cells):
                    with st.expander(f"Celda {i+1} ({cell.get('cell_type', 'desconocido')})", expanded=i < 3):
                        display_notebook_cell(cell)
                
                if len(notebook.get('cells', [])) > 10:
                    st.info(f"Mostrando 10 de {len(notebook.get('cells', []))} celdas. Descarga el notebook completo para ver todo el contenido.")
