...
class DataGuardianAgent(BaseAgent):
    ...
    def resolve_target(self, df: pd.DataFrame, intent: Dict) -> str:
        ...
        self.ctx.document_operation(
            ...
            f"La columna **{best_target}** ha sido seleccionada como objetivo. A continuación se presentan sus estadísticas básicas:\n\n{target_info_str}",
    ...
    def _execute_pipeline(self, df: pd.DataFrame, target: str) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if pd.api.types.is_object_dtype(y) and not pd.api.types.is_numeric_dtype(y):
            ...
            self.ctx.document_operation(
                ...
                f"Mapeo de clases:\n" + 
                "\n".join([f"- Clase original: '{original}' → {encoded}" 
...
class ModelShamanAgent(BaseAgent):
    ...
    def _train_and_evaluate(self, model: BaseEstimator, X: pd.DataFrame, y: pd.Series, 
        ...
        # Documentar la división de datos
        self.ctx.document_operation(
            "División en Conjuntos de Entrenamiento y Prueba",
            ...
            + (f"*Nota: Se ha utilizado estratificación para mantener la distribución de clases.*" 
               if problem_type == 'classification' else ""),
            "# División en conjuntos de entrenamiento y prueba\n"
            "from sklearn.model_selection import train_test_split\n"
            "X_train, X_test, y_train, y_test = train_test_split(\n"
            f"    X, y, test_size={CONFIG['DEFAULT_TEST_SIZE']}, random_state={CONFIG['RANDOM_SEED']}," +
            ("\n    stratify=y  # Mantener distribución de clases" if problem_type == 'classification' else "") +
            "\n)\n"
